...
DEFAULT_TEMPLATE_PATH = str(_SCRIPT_DIR / "templates" / "viewer.html")
OUTPUT_BASE_DIR = str(_SCRIPT_DIR.parent.parent / "outputs")
...
def main() -> None:
    ...
    ppt_dir = os.path.join(output_dir, "final_output")
    os.makedirs(ppt_dir, exist_ok=True)
    safe_title = "".join(
        c if c.isalnum() or c in " -_" else "_"
        for c in slides_plan.get("title", "presentation")
    ).strip() or "presentation"
    pptx_path = os.path.join(ppt_dir, f"{safe_title}.pptx")
    print("Converting slides to PPTX...")
    convert_to_pptx(os.path.join(output_dir, "images"), pptx_path)
